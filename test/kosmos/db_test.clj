...
(start! (initialize (dissoc config :db)))

(fact "database component with pooling working"

  (let [ds (:ds system)]
    (jdbc/execute! ds ["drop table test if exists"]) => irrelevant
    ...
    (jdbc/insert! ds :test {:name "test-01" :description "a test record one"}) => irrelevant
    (jdbc/insert! ds :test {:name "test-02" :description "a test record two"}) => irrelevant
    (jdbc/insert! ds :test {:name "test-03" :description "a test record three"}) => irrelevant
    (jdbc/insert! ds :test {:name "test-04" :description "a test record four"}) => irrelevant))

(stop!)

(start! (initialize (dissoc config :ds)))

(fact "database component without pooling working"
